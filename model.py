...
class MF(Model):
    ...
    def mf_block(self):
        ...
        user_bias = tf.compat.v1.get_variable("user_bias", [n_user],
                                    initializer=tf.constant_initializer(0))
        item_bias = tf.compat.v1.get_variable("item_bias", [n_item],
                                    initializer=tf.constant_initializer(0))

        i_b = tf.nn.embedding_lookup(item_bias, i)
        u_b = tf.nn.embedding_lookup(user_bias, u)
        b = tf.compat.v1.get_variable("global_bias", [],
                            initializer=tf.constant_initializer(0))
    ...
    def model_constructor(self):
        ...
        tf.compat.v1.disable_v2_behavior()
        ai = tf.compat.v1.placeholder(tf.int32, [None])
        au = tf.compat.v1.placeholder(tf.int32, [None])
        aui = tf.compat.v1.placeholder(tf.int32, [None])
